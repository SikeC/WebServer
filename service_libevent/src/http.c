...
void http_send_headers(char *type, off_t length,bufferevent *bev)
{
    char tmp_out[4096];
    int i;
    i = sprintf(tmp_out, "%s %d %s\r\n", "HTTP/1.1", 200, "0k");
    i = sprintf(tmp_out+i, "Content-Type:%s\r\n", type);
    i = sprintf(tmp_out+i, "Content-Length:%ld\r\n", (int64_t)length);
    i = sprintf(tmp_out+i, "Connection: close\r\n");
    i = sprintf(tmp_out+i, "\r\n");

    bufferevent_write(bev,tmp_out,strlen(tmp_out));
}

void http_send_err(int status, char *title, char *text,bufferevent *bev)
{
    char tmp_out[4096];
    int i;

    http_send_headers("text/html", -1, bev);

    i = sprintf(tmp_out, "<html><head><title>%d %s</title></head>\n", status, title);
    i = sprintf(tmp_out+i, "<body bgcolor=\"#cc99cc\"><h4>%d %s</h4>\n", status, title);
    i = sprintf(tmp_out+i, "%s\n", text);
    i = sprintf(tmp_out+i, "<hr>\n</body>\n</html>\n");
...
void *deal_http(void *arg)
{
    ...
    memcpy(buf, msg->buf, sizeof(msg->buf));
